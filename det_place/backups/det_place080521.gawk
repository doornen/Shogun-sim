...
function resetPos(i, \
    ...
    det[i,"xd","x"] = 1;   # intrinsic x-direction in the laboratory
...
#  print sxy
    ...
    det[i,"tp-tc"] = tc;  # theta center
    det[i,"tp-pl"] = pl;  # phi low
    det[i,"tp-ph"] = ph;  # phi high
    det[i,"tp-pc"] = pc;  # phi center
...
function placeDet (i, d, th, ph) {  # place detector i at distance d and angles theta and phi
    transDet(i,"z",d);
    rotDet(i,"y",th);
    rotDet(i,"z",ph);
}
...
#    for (i=1; i<=numDet; i++) resetPos(i);
    ...
    while (id <= numDet && is <= numStep) {   # while there are still detectors and rings to fill
        ...
        if (rr < rrOrig/2) break;                   # no space leaft for another ring
...
# check if there are n detectors of the same type available
        ...
        thRing[iRing]  = th;
        tlRing[iRing]  = tl;
        scRing[iRing]  = 0.5 * (cos(tl) - cos(th))  # solid angle coverage (= solidangle / 4pi)

        print "Placing ", n, " detectors in ring", iRing, " theta coverage (",tl/DEG,",",th/DEG,"). Used ",iPlacementSteps," steps so far.";
        while ( id < ii && id <= numDet ) {
            det[id,"r"] = iRing;
            det[id,"u"] = 1;
            det[id,"d"] = d;
            det[id,"th"] = t;
            det[id,"ph"] = pc * ( id - ii + n );
            id++;
        }
...
#  print "i | x y z | xd1-3 | yd1-3 | zd1-3 | theta phi";
    ...
    for (i=1; i<=numDet; i++) {
        if ( ! det[i,"u"] ) continue;
        resetPos(i);
        placeDet(i, det[i,"d"], det[i,"th"], det[i,"ph"]);
