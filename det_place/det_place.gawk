...
# 2009-10-27  HS  added usage output (use -? or -help)
#                 added verbosity option (use -v to get old messages)
#                 option to specify detector size on commandline
#                 output setup information before running (if -v option is used)
#                 describe values in plot (red and blue points)
#
# todo:
#   
...
# type 5: possible LaBr or LaCl size
    ...
    typeSpec[t,"sy2"] = 4;
    typeSpec[t,"sz"] = 8;
# type 6: possible LaBr or LaCl size
    t = 6;
    typeSpec[t,"sx1"] = 1.5;   # size in x, y, z  (somewhat longer than type 4)
    typeSpec[t,"sx2"] = 1.5;   
    typeSpec[t,"sy1"] = 4;
...
# type 7: possible LaBr or LaCl size
    t = 7;
...
function usage() {
    ...
    print "   -a t r for theta>t use angular resolution r (in degrees)"
    print "                        or energy resolution delta-e (negative values in %)"
    print "   -b b   specify beam velocity (beta) (default 0.6)"
    print "   -w w   specify wall thicknes of detector housing (default 1 mm)"
    print "   -g t g for theta>t group detector in numbers of g"
    print "   -t t n use n detectors of type t"
    print "   --addtype  t-num x1 x2 y1 y2 z"
    print "          add detector type (will overwrite existing types)" 
    print ""
    print ""
    print ""
    print ""
...
# start parsing the command line    
    ...
    for (i = 1; i < ARGC; i++) {
        ...
        if ( 0 ) {
        } else if ( ARGV[i] == "-?" || ARGV[i] == "--help" || ARGV[i] == "-help" ) {        # help
            usage();
            exit 0;
        ...
        } else if ( ARGV[i] == "--addtype" ) {
            t = ARGV[++i];
            typeSpec[t,"sx1"] = ARGV[++i];   # size in x, y, z
            typeSpec[t,"sx2"] = ARGV[++i];   
            typeSpec[t,"sy1"] = ARGV[++i];
            typeSpec[t,"sy2"] = ARGV[++i];
            typeSpec[t,"sz"]  = ARGV[++i];
...
#           90 type 2  DALI2
#
        print "# No detector type specified; using all DALI1/2 detectors"
        numType++;
        type[numType,"t"] = 3
        type[numType,"n"] = 6
        numType++;
    ...
    if (verb) {
        printf "Beta           = %.2f\n", beta
        printf "Wall thickness = %.2f cm\n", wall
        print "Angular (energy) resolution"
        for (i=1; i<=numStep; i++) {
            printf "%2d: from %6.1f deg ", i, ang[i,"l"]/DEG
            if (ang[i,"r"]/DEG >0) {
                printf "%5.1f deg (angular resolution)\n", ang[i,"r"]/DEG
            } else if (ang[i,"r"]/DEG < 0){
                if (beta > 0) {
                    printf "%5.1f % (energy resolution)\n", -ang[i,"r"]/DEG
                } else {
                    print
                    print "beta must be larger than zero if energy resolution is specified; exiting"
                    exit 1;
                }
            }
        }

        print "Grouping of detectors"
        for (i=1; i<=numGrpStep; i++) {
            printf "%2d: from %6.1f deg grouped in %2d\n", i, grp[i,"l"]/DEG, grp[i,"n"];
        }

        printf "First "
        for (i = 1; i<=numType; i++) {
            typeTotal[type[i,"t"]] += type[i,"n"];
            printf "using %5d detectors of type %3d.\n", type[i,"n"], type[i,"t"];
            printf "then  "
        }
        print
        
        for (t in typeTotal) {
            printf "Using a total of %5d detectors of type %3d (size: x1/2=%.1f %.1f   y1/2=%.1f %.1f   z=%.1f).\n", typeTotal[t], t,\
                typeSpec[t,"sx1"],\
                typeSpec[t,"sx2"],\
                typeSpec[t,"sy1"],\
                typeSpec[t,"sy2"],\
...
# setup detectors (i.e. fill the array det[])
    detSetup();
    if (verb) print "Done with setup.  Now positioning the detectors.";

# start with positioning  (this is the main part of this script)
...
#    for (i=1; i<=numDet; i++) resetPos(i);
    ...
    while (id <= numDet && is <= numStep) {   # while there are still detectors and rings to fill
        if (verb) print "Starting with ring ", iRing+1, ".  Working on detector", id, "."
        ...
        if (rr < 0) {   # ang[is,"r"] is the energy resolution in %FWHM, convert to angle
            enRes = - 0.01 * rr/DEG;
            enRes *= gr;   # increase by the grouping number 
            ...
            rr *= gr;  # increase by grouping number
        }
        if (deb) print "ll, rr", ll/DEG, rr/DEG;
        rrOrig=rr;
        ...
        if (deb) print "Aiming for angular resolution of ", rr/DEG, " deg.";
        ...
        if (deb) printf "%s", "Need to place " n*gr " detectors.  ";
        if (id + n*gr - 1 > numDet) {
...
# check if there are n detectors of the same type available
        if ( det[id,"t"] != det[id + (n * gr) - 1,"t"] ) {
            print "# Don't have enough detectors. (rerun with -v option to see which detectors are skipped)";
            ii = id;
            while ( det[ii,"t"] == det[id,"t"] && id <= numDet ) {
                if (verb) print "Skipping id", id, ".";
                detSkip[det[id,"t"]]++;
                id++;
            }
            continue;
        ...
        for (i=1; i<=gr; i++){
            ...
            if (verb) print "Solid angle coverage of ring ", iRing, " is ", scRing[iRing]

            if (deb) print "Placing ", n, " detectors in ring", iRing, " theta coverage (",tl/DEG,",",th/DEG,"). Used ",iPlacementSteps," steps so far.";
            iii = id + n;
            while ( id < iii && id <= numDet ) {
                det[id,"r"] = iRing;
                det[id,"u"] = 1;
                det[id,"gr"] = gr;   # number of detectors in groups
                det[id,"gri"] = i;  # number of detector in this group  (e.g.: gr=2; gri=1 => first detector in a group of 2)
                det[id,"d"] = d;
                det[id,"x0"] = x0;
                det[id,"th"] = t;
...
#  print "i | x y z | xd1-3 | yd1-3 | zd1-3 | theta phi";
    print "#          |           ----  center coordinates   ---- | -----   crystal size        -----|--- group ---";
    print "# det ring type  x     y      z  |   r    theta    phi |  sx1    sx2    sy1    sy2    sz     size    num ";
    for (i=1; i<=numDet; i++) {
        if ( ! det[i,"u"] ) continue;
        resetPos(i);
        placeDet(i, det[i,"d"], det[i,"th"], det[i,"ph"], det[i,"x0"]);
        thetaPhiCover(i,"dummy");
        printf "%3d %3d %4d %6.2f %6.2f %6.2f %6.2f %6.2f %6.2f %6.2f %6.2f %6.2f %6.2f %6.2f %6d %6d\n", \
...
#  print "set xrange [-50:100]" >gpf;
    ...
    print "" >gpf;
    print "set label \"{/=24 SHOGUN Configuration}\" at screen .5,.98 cent" >gpf;
    ...
    print "set label \"{/=10 Red (green) things: input parameters (from command line)\" at screen .1,-.01 tc lt 1" > gpf;
    xpos = .1;
    ypos = .93;
    for (t in detSkip) {
    ...
    for (i=1; i<=numStep-1; i++) {
        ...
        print "set label \"{/=10 -a" t/DEG, "," ang[i,"r"]/DEG "}\" at ",r*cos(t), ",", r*sin(t) ,"rot by ", t/DEG " tc lt 1" >gpf;
    }
    for (i=1; i<=numGrpStep-1; i++) {
        r = 14;
        t = grp[i,"l"];
        print "set arrow from 0,0 to ",r*cos(t), ",", r*sin(t), "lt 2 lw 2 nohead" > gpf;
        print "set label \"{/=10 -g" t/DEG, "," grp[i,"n"] "}\" at ",r*cos(t), ",", r*sin(t) ,"rot by ", t/DEG " tc lt 2" >gpf;
    }
    for (i=1; i<=numDet; i++) {
        if ( ! det[i,"u"] ) continue;
...
#  print "set arrow from graph 0,0 rto 10,0 lc 7 lw 2 nohead" >gpf;   # rto does not seem to work
    ...
    print "set label \"{/=8 Solid ang. abs/ratio}\" at graph -.12,.5 rot cent" > gpf;
    print "set label \"{/=8 Resolution (FWHM)}\" at graph 1.1,.5 rot cent point lt 7 pt 7 ps .5" > gpf;
    print "set label \"{/=7 red: ratio of covered to total SA per ring}\" at graph .1,.1 left" >gpf;
    print "set label \"{/=7 green/blue: SA/SA boosted}\" at graph .9,.1 right" >gpf;
    print "set key off" >gpf;
    print "plot '-' tit 'ratio of covered sa' w l lt 1, '-' tit 'sa coverage'w lp lt 2 pt 1, '-' tit 'boosted sa coverage' w lp lt 3 pt 1, '-' tit 'res.' axis x1y2 w p lt 7 pt 7 ps .5" > gpf;
    scRingMax = 0;
    scRingBoostMax = 0;
    scRingCoverMax = 0;
    for (i = 1; i <= numOfRings; i++) {
        v = scRing[i];
        if (v > scRingMax) scRingMax = v;
        v = scRing[i]*boostFactor[i];
        if (v > scRingBoostMax) scRingBoostMax = v;
        v = solidAngle[i]/scRing[i];
        if (v > scRingCoverMax) scRingCoverMax = v;
    }
    for (i = 1; i <= numOfRings; i++) {
    ...
    for (i = 1; i <= numOfRings; i++) {
        t = tRing[i];
        print t/DEG, scRing[i]*scRingCoverMax/scRingMax >gpf;
    }
    print "e" > gpf;
    for (i = 1; i <= numOfRings; i++) {
        t = tRing[i];
        print t/DEG, scRing[i]*boostFactor[i]*scRingCoverMax/scRingBoostMax >gpf;
    }
    print "e" > gpf;
    for (i = 1; i <= numOfRings; i++) {
        t = tRing[i];
