...
void EfficiencyFitterStopped(int caseNumber)  {
  //c2->Divide(3,5);
  //c2->cd();
  TCanvas *fcanvas;  
  char dummytext[1000];
  ...
  //TH1F *h2[12][14];
  //TH1F *h2;
  sprintf(dummytext,"./output/FittingResults/Case%iStopped.txt",caseNumber);
  ...
  for(int j=500;j<=3000;j+=500)  {
    ...
    cout<<"Reading Root-tree"<<j<<endl;
    
    if(j<1000)
      sprintf(root_file,"../../../SimulationResults/Reconstructor/Shogun/Efficiency/Case%i_%ikeVStopped.root",caseNumber,j);
    else
        sprintf(root_file,"../../../SimulationResults/Reconstructor/Shogun/Efficiency/Case%i_%ikeVStopped.root",caseNumber,j);
    ...
    Double_t FRes[100];
    RT_FitManPar(h[k],CPos,FRes,0,1,1,0,0,j);
