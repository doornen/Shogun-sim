...
void EfficiencyFitter(int caseNumber)  {
  ...
  //TH1F *h2[12][14];
  //TH1F *h2;
  sprintf(dummytext,"./output/FittingResults/Case%i.txt",caseNumber);
  ...
  for(int i=100;i<=250;i+=50)  {
    for(int j=250;j<=3000;j+=250)  {
      ...
      sprintf(root_file,"../../../SimulationResults/Reconstructor/Shogun/Efficiency/Case%i_%iMeV%ikeV.root",caseNumber,i,j);
