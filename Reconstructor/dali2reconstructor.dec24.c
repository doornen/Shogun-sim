...
int Dali2Reconstructor()  {
  ...
  // Ringwise:
  //for(int i=0;i<17;i++)  {
  //  sprintf(temp,"doppler_ring[%i]",i);
  //  h_doppler_ring[i] = new TH1F(temp,temp,numBin,firstBin,lastBin);
  //}
  ...
  if(dali2_addback_opt==1)  {
    ...
    for(int i=0;i<NUMBEROFDALI2CRYSTALS;i++)  {
      ...
      for(int j=0;j<NUMBEROFDALI2CRYSTALS;j++)  {//Modified by Ryo Taniuchi to make appropriate add-back Oct16
	if(j==i) continue;
        for(int k=0;k<3;k++) {
	  if(dali2_fi_opt==2){//Dec24 Chen
      ...
      //for(int j=counter+1;j<NUMBEROFDALI2ADDBACKCRYSTALS;j++){
      for(int j=counter;j<NUMBEROFDALI2ADDBACKCRYSTALS;j++){
	addbackTable[i][j] = -1;//SC RT
      }
      counter = 0;
  ...
  //Dividing new dali2.1 into 17 rings of equal theta angle:
  ...
  int ringLow[17] = {0, 6,12,20,30,40,52,66,80, 94,108,122,199,199,199,199,199};
  int ringHigh[17]= {5,11,19,29,39,51,65,79,93,107,121,185,199,199,199,199,199};
  ...
  for(int iii=0;iii<nentries;iii++)  {
    ...
    for(int nnn=0;nnn<NUMBEROFDALI2CRYSTALS;nnn++)  {
      //    if(nnn==0  || nnn==1  || nnn==2  || nnn==3 || nnn==4  || nnn==6  || nnn==39 || nnn==63 || 
      //     nnn==123 || nnn==124|| nnn==125||nnn==126|| nnn==127|| nnn==130|| nnn==132) continue;//removing crystals that have a bad energy resolutoin.
      if(dali2_flag[nnn]==1)   {
    ...
    if(eventNumberNext!=evnum)  {                 // The last observed decay we can start to fill the spectra
      ...
      //SortEnergies();           //Sort the observed energies from highest to lowest!!!! // Function undefined.
      ...
      //////////////////////////////////////////
      //////// Modified By RyoTaniuchi /////////
      //////////////////////////////////////////
      if(crystalMult>=1 && dali2_addback_opt==1)  {
	for(int l = 0;l<crystalMult;l++)  {// Just do same procedure several times. //RT
	  float HighestEnergy = 0.; //RT
	  int HighestCrystal =-1; //RT, SC
	  for(int i = 0;i<crystalMult;i++)  {// Search the highest energy within unused crystals//RT
	    if(crystalUsedForAddback[crystalFired[i]]==true) continue;
	    if(HighestEnergy < energy[crystalFired[i]])  {
	      HighestEnergy = energy[crystalFired[i]];
	      HighestCrystal = crystalFired[i];
	    }
	  }
	  if(HighestCrystal<0) break; //RT SC
	  //for(int i = 0;i<crystalMult;i++)  {
	  //if(crystalUsedForAddback[crystalFired[i]]==true) continue;
	  //float dummyEnergy = dopplerEnergy[i]; // This value is AFTER the doppler correction
	  //crystalUsedForAddback[crystalFired[i]]=true; 

	  float dummyEnergy = HighestEnergy;//RT
	  crystalUsedForAddback[HighestCrystal]=true; //RT

	  //for(int j = i;j<crystalMult;j++)  {//All crystals should be compared every time. Because this is not sorted by Energy.
	  for(int j = 0;j<crystalMult;j++)  {//RT
		...
		if(addbackTable[HighestCrystal][k]<0) continue; //To avoid too much loops. SC RT
		if(crystalFired[j] == addbackTable[HighestCrystal][k])  {
		  ...
		  dummyEnergy = dummyEnergy + energy[crystalFired[j]]; //RT in lab frame.
		}
	      }
	    }
	  }
	  //Doppler Reconstruction //RT
          float AdBkEnergy=0.0;
          //dali2_fi_opt==2 I use the average FI point from the simulation
          if(dali2_fi_opt !=2)
            AdBkEnergy = GetDopplerCorrectedEnergy(dali2_pos[0][HighestCrystal],dali2_pos[1][HighestCrystal],dali2_pos[2][HighestCrystal],
                                                               ver_rec[0],ver_rec[1],ver_rec[2],pos_det[0],pos_det[1],pos_det[2], 
                                                               dummyEnergy,beta_rec,beta_mean_tof,beta_average,decay_z);
          else
            AdBkEnergy = GetDopplerCorrectedEnergy(dali2_pos_ave[0][HighestCrystal],dali2_pos_ave[1][HighestCrystal],dali2_pos_ave[2][HighestCrystal],
                                                               ver_rec[0],ver_rec[1],ver_rec[2],pos_det[0],pos_det[1],pos_det[2], 
                                                               dummyEnergy,beta_rec,beta_mean_tof,beta_average,decay_z); 
	  dummyEnergy=AdBkEnergy; //RT
	  h_doppler_addback[0]->Fill(dummyEnergy);
	  //if(posThatsIt[2]<0) {
	  if(dali2_pos_ave[2][HighestCrystal]<0){
      ...
      if(crystalMult>=2)  {
        for(int i = 0;i<crystalMult;i++)
          for(int j = i+1;j<crystalMult;j++){
            //if (dopplerEnergy[i]>=dopplerEnergy[j])          //modified by momiyama on 150630
              h_doppler_gamma_gamma->Fill(dopplerEnergy[i],dopplerEnergy[j]);
              h_doppler_gamma_gamma->Fill(dopplerEnergy[j],dopplerEnergy[i]);
